...
docker build --rm -f "Dockerfile" -t "$DOCKER_USRENAME/youtube2podbean:$TRAVIS_TAG" .
...
docker build --rm -f "services/discord.Dockerfile" -t "$DOCKER_USRENAME/youtube2podbean-discord:$TRAVIS_TAG" --build-arg YOUTUBE2PODBEAN_SERVICE=app.services.discord ..
...
docker build --rm -f "services/podbean.Dockerfile" -t "$DOCKER_USRENAME/youtube2podbean-podbean:$TRAVIS_TAG" --build-arg YOUTUBE2PODBEAN_SERVICE=app.services.podbean ..
...
docker build --rm -f "services/wordpress.Dockerfile" -t "$DOCKER_USRENAME/youtube2podbean-wordpress:$TRAVIS_TAG" --build-arg YOUTUBE2PODBEAN_SERVICE=app.services.wordpress ..
...
docker build --rm -f "services/youtube.Dockerfile" -t "$DOCKER_USRENAME/youtube2podbean-youtube:$TRAVIS_TAG" --build-arg YOUTUBE2PODBEAN_SERVICE=app.services.youtube ..
